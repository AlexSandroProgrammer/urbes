...
if ((isset($_POST["MM_FormUpDispocision"])) && ($_POST["MM_FormUpDispocision"] == "FormUpDispocision")) {
    ...
    // Manejo de imágenes
    $foto_km_inicio = $_FILES['foto_km_inicio'];
    $foto_km_fin = $_FILES['foto_km_fin'];
    ...
    try {
        ...
        // Actualizar los datos de la disposición
        ...
        // Manejar las imágenes si son subidas
        ...
        if ($foto_km_inicio_name) {
            move_uploaded_file($foto_km_inicio['tmp_name'], "../assets/images/" . $foto_km_inicio_name);
        }

        if ($foto_km_fin_name) {
            move_uploaded_file($foto_km_fin['tmp_name'], "../assets/images/" . $foto_km_fin_name);
        }
        ...
        // Redirigir con éxito
        showErrorOrSuccessAndRedirect("success", "Actualización Exitosa", "Los datos de la disposición se han actualizado correctamente", "index.php");

    } catch (Exception $e) {
        // Revertir la transacción en caso de error
        $connection->rollBack();
        showErrorOrSuccessAndRedirect("error", "Error de Actualización", "No se pudo actualizar los datos de la disposición: " . $e->getMessage(), "index.php");
    }
}
?>