...
if ((isset($_POST["MM_formUpdateEmployee"])) && ($_POST["MM_formUpdateEmployee"] == "formUpdateEmployee")) {
    ...
    if (containsSpecialCharacters([
        ...
        showErrorOrSuccessAndRedirect(
            ...
            "editar_socio.php?id_employee-edit=" . $documento . "&ruta=" . $ruta
        );
        exit();
    }
    // ID DEL SOCIO
    ...
    if ($resultValidation) {
        ...
        showErrorOrSuccessAndRedirect("error", "Error de registro", "Por favor revisa los datos ingresados, porque los datos registrados ya pertenecen a otro usuario.", "editar_socio.php?id_employee-edit=" . $documento . "&ruta=" . $ruta);
        exit();
    } else {
        try {
            // fecha actual
            ...
            $editarDatosSocio->bindParam(':nombre_familiar', $nombre_familiar);
            $editarDatosSocio->bindParam(':celular_familiar', $celular_familiar);
            $editarDatosSocio->bindParam(':parentezco_familiar', $parentezco_familiar);
            $editarDatosSocio->bindParam(':eps', $eps);
            $editarDatosSocio->bindParam(':arl', $arl);
...
if (isset($_GET['id_employee-delete'])) {
    ...
    if (isEmpty([$id_employee])) {
        ...
        $deleteSocio = $connection->prepare("SELECT * FROM usuarios WHERE documento = :id_employee");
        $deleteSocio->bindParam(":id_employee", $id_employee);
        $deleteSocio->execute();
