...
if ((isset($_POST["MM_formRegisterZone"])) && ($_POST["MM_formRegisterZone"] == "formRegisterZone")) {
    ...
    $estadoQueryFetch = $connection->prepare("SELECT * FROM zonas WHERE zona= :zona");
    ...
    if ($queryFetch) {
        ...
        $registerZona = $connection->prepare("INSERT INTO zonas(zona, fecha_registro) VALUES(:zona, :fecha_registro)");
...
if ((isset($_POST["MM_formUpdateZone"])) && ($_POST["MM_formUpdateZone"] == "formUpdateZone")) {
    ...
    // validamos que no se repitan los datos del nombre del actividad
    // CONSULTA SQL PARA VERIFICAR SI EL REGISTRO YA EXISTE EN LA BASE DE DATOS
    $zoneQueryUpdate = $connection->prepare("SELECT * FROM zonas WHERE zona = :zona AND id_zona <> :id_zona");
    ...
    if ($queryZone) {
        ...
        $updateZone->bindParam(':id_zona', $id_zona);
        $updateZone->execute();
        if ($updateZone) {
